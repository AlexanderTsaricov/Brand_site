function addProductTeg(product) {
    ...
    teg_a.innerHTML = `
        <a href="#" class="productCartBox_cartBox_cart_hover_payButton" id=${product.id}>
...
if (document.title == "Catalog") {
    ...
    for (let index = 0; index < allProducts.length; index++) {
        cartBox.appendChild(addProductTeg(allProducts[index]));
    }
    addNumbersTag(allProducts.length - 8, 1);
    ...
    function maxWidth951(media) {
        if (media.matches) {
            ...
            productOnPage = 0;
            minProduct = 8;
            let maxProducts = Math.min(minProduct, AllProducts.length);
            for (let index = 0; index < maxProducts; index++) {
                cartBox.appendChild(addProductTeg(AllProducts[index]));
                productOnPage++;
            }
            removeTags("catalogListNumber");
            remainderCart = AllProducts.length - 7;
    ...
    function add9cart(media) {
        if (media.matches) {
            ...
            removeTags("catalogListNumber");
            remainderCart = allProducts.length - 8;
            addNumbersTag(remainderCart, 1);
        }
    }
